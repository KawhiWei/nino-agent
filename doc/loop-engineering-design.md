...
责任：直接回答、能力选择、派发、消费子结果、继续派发或结束。

```mermaid
stateDiagram-v2
    [*] --> BeforeModel
    BeforeModel --> Decide
    Decide --> Completed: final text
...
推理或 DeepSeek `reasoning_content`。
...
- Orchestrator direct answer 与 dynamic dispatch。
