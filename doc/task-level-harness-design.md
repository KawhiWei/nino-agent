...
> 适用版本：Nino Agent Runtime v0.15.0
>
> 已提交基线：`facbc9e feat: 拆分 Planner 并统一业务中立 Harness 架构`；当前设计：Runtime `0.15.0`
>
...
User Request
  ...
  -> Orchestrator Validate / Persist / Schedule
  -> Generic Analyst ReAct Worker
  -> MCP Tool Observation
  -> Evidence Gate
  -> Generic Verifier Re-query
...
Advisory Plane
  - decompose goal
  - propose candidate nodes/dependencies/bindings/contracts
...
flowchart TB
    ...
    Orchestrator --> Planner["PlannerHarness"]
    Planner --> Catalog["AgentRegistry + SkillRegistry"]
    Orchestrator --> Scheduler["TaskGraphScheduler"]
    Orchestrator --> Analyst["nino.analyst ReAct Worker"]
    Orchestrator --> Verifier["nino.verifier ReAct Worker"]
    Planner --> Model["ChatModel Port"]
    Orchestrator --> Model
    Analyst --> Model
    Verifier --> Model
    Analyst --> References["ReferenceProvider"]
    Verifier --> References
...
sequenceDiagram
    ...
    participant P as PlannerHarness
    participant W as Generic Analyst Worker
    ...
    O->>P: request candidate TaskGraph revision
    P-->>O: structured candidate nodes only
    O->>O: deterministically validate proposal
    ...
    O->>O: reconcile verified node results without tools
    O-->>R: final RunResult
...
| `nino.planner` | 用户目标、会话历史、候选能力元数据、紧凑 Node outcome | 候选节点、依赖、binding、AcceptanceContract，或澄清/拒绝 | 持久化、调度、MCP、最终回答 |
...
| 对象 | 回答的问题 | 当前实现 |
|---|---|---|
| Agent | 谁承担这个节点的职责？ | orchestrator、planner、generic analyst、generic verifier |
...
同一个 logical node 重新出现时：

```text
fingerprint 相同
  -> 复用原 Completed Node Result，不创建 Attempt

fingerprint 不同
  -> 保留旧 Completed 的 status/result/gate
  -> 创建带新物理 ID 的 Node
  -> 新 Node.supersedes_node_id 指向旧 Node
```
...
Planner 在失败后使用新的 repair logical ID 时，必须显式提交 `supersedes_node_id`。Orchestrator 只
允许它指向早期 revision 中失败或 blocked 的节点，并禁止 repair 同时依赖被替代节点。TaskGraph
Controller 接受 revision 后，将旧节点和所有尚未 Completed 的受影响下游标记为 `superseded`，Gate
标记 `blocked`，并保存 `superseded_by_node_id/invalidated_by_node_id`。
...
## 11. Generic Analyst/Verifier Worker ReAct
...
global MCP catalog
  ...
  intersect Agent role compatibility policy
...
at-least-once Root replay + fingerprint-safe completed-node reuse。
...
| `test_orchestrator.py` | Planner/Orchestrator 分离、无 MCP 规划、DAG、并行、binding、reconcile、Verifier 和节点复用 |
...
```text
nino.planner planning
  -> candidate node order_financials_demo_202607_001
Orchestrator accepts graph_planned revision
  -> nino.analyst loads Skill/References and calls nino_data_get_order_detail
  -> evidence Gate passed
  -> nino.verifier independently re-queries and submits proved/pass
  -> independent_verification Gate passed
  -> nino.orchestrator reconciliation (tools = empty)
  -> TaskGraph completed -> Run completed
```
...
`22.1` 至 `22.6` 来自已提交 Git 历史；`0.15.0` 描述当前工作区设计，提交后应以对应提交号替换版本标签。
