...
- Orchestrator 从动态能力目录选择兼容的 Agent + Skill，普通问题可直接回答。
...
1. 主模型读取能力摘要并直接回答或调用 `nino_runtime_dispatch_agent`。
...
- `intent_keywords` 仅用于底层无 Orchestrator fallback；通用入口使用能力语义路由。
...
| `NINO_MODEL_NAME` | OpenAI-compatible 模型 ID |
| `NINO_MODEL_API_KEY` | 模型密钥 |
| `NINO_MODEL_BASE_URL` | 兼容 `/v1` endpoint |
