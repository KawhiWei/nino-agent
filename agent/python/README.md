...
| 动态 Skill 路由 | 已实现 | 确定性排除和关键词召回；未命中时只允许显式 opt-in 的语义 fallback |
...
路由先执行确定性排除和关键词召回。关键词命中会直接限制候选目录；未命中时只有
`semantic_fallback=true` 的 Skill 可进入模型辅助路由。Planner 可提出有边界的 TaskGraph Node、澄清、
