...
严格边界由 Harness 执行：`intent_keywords` 只召回 Skill 明确支持的意图；未命中时，只有声明
`semantic_fallback=true` 的 Skill 可以进入模型辅助路由。Planner 必须依据候选的 `capabilities`
判断是否支持请求，没有任何能力匹配时结构化拒绝。
...
### 可继续工程化的 Skill / Agent 场景

下列业务仍属于“只读取证 + 分析 + 独立验证”，优先复用现有 `nino.analyst` 和 `nino.verifier`，只新增
Skill、Reference、MCP Tool 和固定题库：

| 场景 | 可落地能力 | 需要补齐的工程契约 |
|---|---|---|
| 客诉定位 | 汇总订单、退款、履约时间线并指出证据缺口 | 客户与订单查询 Tool、隐私字段脱敏、客诉 Golden Case |
| 供应商结算 | 对比应付、实付、退款回收和差异原因 | 结算 Tool、金额口径 Reference、差异阈值题库 |
| 退款对账 | 关联支付、客户退款、供应商追回并输出未闭环项 | 对账 Tool、状态机 Reference、重复退款边界题 |
| 渠道盈利分析 | 比较渠道收入、成本、退款、毛利和趋势 | 分组聚合 Tool、时间口径、跨周期确定性真值 |
| SLA 与异常巡检 | 找出超时、失败、重试过多的业务记录 | 只读事件 Tool、SLA 规则、Top N 与总数断言 |
| 合规证据查询 | 按主体和时间范围生成只读证据清单 | 审计 Tool、字段级权限、证据完整性题库 |

只有职责或验收语义变化时才新增 Agent：

| 候选 Agent | 适用场景 | 新增前置条件 |
|---|---|---|
| `nino.reviewer`（审阅 Agent） | 检查报告结构、披露项和输出合同 | 实现 `review` evaluator 和确定性审阅合同 |
| `nino.critic`（质疑 Agent） | 反向检查假设、偏差和证据不足 | 实现 `critique` evaluator 与可评分的质疑规则 |
| `nino.approver`（审批协调 Agent） | 高风险动作前收集人工批准 | 真实审批 API、身份校验、超时和拒绝状态机 |
| `nino.writer`（制品生成 Agent） | 生成可下载报表或审计包 | 制品存储、版本、校验和与下载权限 |
| `nino.operator`（操作 Agent） | 执行退款、改状态等写操作 | 幂等键、审批、Action Ledger、补偿和重放保护 |
| `nino.monitor`（监控 Agent） | 周期巡检并触发受控任务 | 调度器、持久队列、去重、告警确认和恢复策略 |

当前最适合先落地的是“供应商结算”和“退款对账”两个 Skill；Reviewer、Writer、Operator 等 Agent 在
对应状态、Tool 和审计契约存在之前不应只添加 manifest 占位。
