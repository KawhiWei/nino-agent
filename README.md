...
- Node Fingerprint 覆盖 Agent、Skill 版本、任务、合同、binding 和依赖指纹；只有完全一致的 Completed
  Node 才能复用。
- repair Node 使用 `supersedes_node_id` 建立 revision lineage，并将受旧节点影响的未完成下游标记为
  `superseded`，Completed 历史结果保持冻结。
...
先启动 Python Runtime，再启动 React 客户端：
...
重复提交用户消息。详细配置见
[React Web README](./web/react/README.md)。
